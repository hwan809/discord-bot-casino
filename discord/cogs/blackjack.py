    ...
    async def blackjack(self, ctx: commands.Context, bet: int=DEFAULT_BET):
        ...
        while True:
            ...
            if player_score == 21:  # win condition
                ...
                result = ("블랙잭을 달성했습니다!", 'won')
                break
            elif player_score > 21:  # losing condition
                self.economy.add_money(ctx.author.id, bet*-1)
                result = ("'BUST' (합이 21을넘었다는뜻ㅎ) 되었습니다...", 'lost')
                break
            msg = await out_table(
                title=f"당신의 차례!",
                description=f"당신의 카드 수 합: {player_score}\n" \
                    f"딜러의 카드 수 합: {dealer_score}",
                footer=f'{ctx.message.author.name}'
            )
        ...
        if standing:
            ...
            if dealer_score == 21:  # winning/losing conditions
                self.economy.add_money(ctx.author.id, bet*-1)
                result = ('딜러가 블랙잭을 달성했습니다...', 'lost')
            elif dealer_score > 21:
                self.economy.add_money(ctx.author.id, bet)
                result = ("딜러가 'BUST' (합이 21을넘었다는뜻ㅎ) 되었습니다!", 'won')
