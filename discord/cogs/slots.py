...
class Slots(commands.Cog):
    ...
    async def slots(self, ctx: commands.Context, bet: int=1):
        ...
        #await ctx.send(f'`현재 확률은 {self.PERCENTAGE}% 입니다.`')

        win_rate = self.PERCENTAGE / 100
        ...
        embed = make_embed(
            title=(
                f'{ctx.message.author.name} 티켓 {result[1]}개를 ' + lostwinstr
            ),
            description=(
                f'이제 티켓이 **{self.economy.get_entry(ctx.author.id)[2]}**' +
                '개 남았습니다.'
            ),
            color=(
    ...
    @commands.command(
        brief=f"티켓을 구매해 슬롯머신을 돌리세요! 각 티켓은 ${DEFAULT_BET} 입니다.",
        usage="buyc [credits]",
        aliases=["buy", "b"]
    )
    ...
    @commands.command(
        brief=f'티켓을 판매하세요. 각 티켓은 ${DEFAULT_BET} 입니다.',
        usage="sellc [credits]",
        aliases=["sell", "s"]
    )
    ...
    async def percentage(self, ctx: commands.Context, v1: int):
        if (ctx.message.author.top_role.permissions.administrator):
            await ctx.send(f'`현재 확률은 {self.PERCENTAGE}% 입니다.`')
            self.PERCENTAGE = v1
            await ctx.send(f'`{self.PERCENTAGE}% 로 조정했습니다.`')
        else:
            await ctx.send('되겠냐')
