...
class Economy:
    ...
    def open(self):
        ...
        self.conn = sqlite3.connect('economy.db', timeout=10)
    ...
    def new_entry(self, user_id: int) -> Entry:
        try:
            self.cur.execute(
                "INSERT INTO economy(user_id, money, credits, coupon) VALUES(?,?,?,?)",
                (user_id, 0, 0, 0)
            )
    ...
    @_commit
    def set_coupons(self, user_id: int, credits: int) -> Entry:
        self.cur.execute(
            "UPDATE economy SET coupon=? WHERE user_id=?",
            (credits, user_id)
        )
        return self.get_entry(user_id)
    ...
    @_commit
    def add_coupon(self, user_id: int, coupons_to_add: int) -> Entry:
        coupon = self.get_entry(user_id)[3]
        total = coupon + coupons_to_add
        if total < 0:
            total = 0
        self.set_money(user_id, total)
        return self.get_entry(user_id)
